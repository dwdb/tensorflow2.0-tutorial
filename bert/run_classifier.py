...
flags.DEFINE_integer("predict_batch_size", 18, "Total batch size for predict.")
...
class SimProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        for line in f.readlines():
            ...
            train_data.append(InputExample(guid=guid, text_a=text_a, text_b=None,
                                           label=label))  # 加入到InputExample列表中
            index += 1
        return train_data
    ...
    def get_test_examples(self, data_dir):
        file_path = os.path.join(data_dir, 'test_sentiment.csv')
...
def one_epoch_input_fn(sentences, label_list, batch_size=8, seq_len=64):
    ...
    features = [convert_single_example(i, example, label_list, seq_len, tokenizer)
...
def main(_):
    ...
    if FLAGS.do_predict:
        # def input_examples(sentences):
        sentences = ['衣服太好看了', '这件衣服真好看']
        predict_input_fn = one_epoch_input_fn(sentences, label_list)
        # input_fn = predict_input_fn(sentences, label_list)
        # predict_examples = []
        # for i, sent in enumerate(sentences):
        #     example = InputExample(guid=i, text_a=sent, text_b=None, label=None)
